...
import { LIST_IDS, parseMediaLibraryConnectionListId } from '@common/constants'
import { getMediaLibraryList, getMediaLibraryListByConnection } from '@renderer/utils/ipc'
...
export {
  ...
  getListMusics as getNormalListMusics,
  removeListMusics,
  updateListMusics,
...
export const getListMusics = async(listId: string | null) => {
  if (listId === LIST_IDS.MEDIA_LIBRARY || listId === LIST_IDS.MEDIA_LIBRARY_ALL) {
    const list = await getMediaLibraryList()
    allMusicList.set(LIST_IDS.MEDIA_LIBRARY_ALL, list)
    allMusicList.set(LIST_IDS.MEDIA_LIBRARY, list)
    return list
  }

  const mediaConnectionId = parseMediaLibraryConnectionListId(listId)
  if (mediaConnectionId && listId) {
    const list = await getMediaLibraryListByConnection(mediaConnectionId)
    allMusicList.set(listId, list)
    return list
  }

  const { getListMusics: getNormalListMusics } = await import('@renderer/store/list/listManage')
  return getNormalListMusics(listId)
}
