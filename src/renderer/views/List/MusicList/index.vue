  ...
  <div :class="$style.list">
    ...
    <div v-show="list.length" ref="dom_listContent" :class="$style.content">
      ...
      >
        <div
          class="list-item"
          :class="[{ [$style.playing]: playerInfo.isPlayList && playerInfo.playIndex === index }, { selected: selectedIndex == index || rightClickSelectedIndex == index }, { active: selectedList.includes(item) }, { disabled: !assertApiSupport(item.source) }]"
        ...
        >
          ...
          <div class="list-item-cell auto name" :class="$style.nameCell" :aria-label="item.name">
            <span class="select name" :class="$style.nameText">{{ item.name }}</span>
            <span v-if="isShowSource" class="no-select label-source">{{ item.source }}</span>
          </div>
          <div class="list-item-cell" :class="$style.metaCell" style="flex: 0 0 22%;"><span class="select" :aria-label="item.singer">{{ item.singer }}</span></div>
          <div class="list-item-cell" :class="$style.metaCell" style="flex: 0 0 22%;"><span class="select" :aria-label="item.meta.albumName">{{ item.meta.albumName }}</span></div>
          <div class="list-item-cell" :class="$style.timeCell" style="flex: 0 0 9%;"><span class="no-select">{{ item.interval || '--/--' }}</span></div>
      ...
      >
        ...
        >
          ...
          <div class="list-item-cell auto name" :class="$style.nameCell">
            <span class="select name" :class="$style.nameText" :aria-label="item.name">{{ item.name }}</span>
            <span v-if="isShowSource" class="no-select label-source">{{ item.source }}</span>
          </div>
          <div class="list-item-cell" :class="$style.metaCell" style="flex: 0 0 25%;"><span class="select" :aria-label="item.singer">{{ item.singer }}</span></div>
          <div class="list-item-cell" :class="$style.metaCell" style="flex: 0 0 28%;"><span class="select" :aria-label="item.meta.albumName">{{ item.meta.albumName }}</span></div>
          <div class="list-item-cell" :class="$style.timeCell" style="flex: 0 0 10%;"><span class="no-select">{{ item.interval || '--/--' }}</span></div>
        </div>
      </base-virtualized-list>
    </div>
...
.list {
  ...
  :global(.list-item) {
    ...
    transition: background-color @ui-transition-fast, color @ui-transition-fast;
