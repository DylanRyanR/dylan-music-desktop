<template>
  <CardShell :title="t('monthly_report__card_top_songs')" :subtitle="t('monthly_report__top_song')">
    ...
    <ul :class="$style.list">
      <li v-for="item in songs" :key="item.songId" :class="$style.item" :title="`${item.songName} | ${item.artistName}`">
      ...
      <li v-if="!songs.length" :class="$style.empty">{{ t('monthly_report__no_data') }}</li>
    </ul>
  </CardShell>
</template>
...
const songs = computed(() => {
  const list = props.cards?.topSongs ?? []
  ...
  const totalMetric = Math.max(1, sorted.reduce((sum: number, item: LX.ReportMonthly.TopSongItem) => {
    return sum + (sortMode.value === 'count' ? item.count : item.seconds)
  }, 0))
  const maxMetric = Math.max(...sorted.map((item: LX.ReportMonthly.TopSongItem) => {
    return sortMode.value === 'count' ? item.count : item.seconds
  }), 1)

  return sorted.map((item: LX.ReportMonthly.TopSongItem, index: number) => ({
    ...item,
    rank: index + 1,
